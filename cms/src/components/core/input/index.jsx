        ...
        return (
          <InputContainer className={`${props.customClass ?? ""} ${props.dynamicClass ?? ""} ${projectSettings.formInputView}`} animation={props.animation}>
            ...
            <div className="relative w-full">
              ...
              {props.onClear && props.value?.toString().length > 0 && (
                <button type="button" onClick={() => props.onClear()} aria-label="Clear" className={`absolute ${props.type === "password" ? "right-9" : "right-3"} top-1/2 transform -translate-y-1/2 text-gray-500 hover:text-gray-700`}>
                  <X size={16} color="#868686" />
                </button>
              )}
            </div>
